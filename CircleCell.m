function varargout = CircleCell(varargin)
% CIRCLECELL MATLAB code for CircleCell.fig
%      CIRCLECELL, by itself, creates a new CIRCLECELL or raises the existing
%      singleton*.
%
%      H = CIRCLECELL returns the handle to a new CIRCLECELL or the handle to
%      the existing singleton*.
%
%      CIRCLECELL('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CIRCLECELL.M with the given input arguments.
%
%      CIRCLECELL('Property','Value',...) creates a new CIRCLECELL or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before CircleCell_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to CircleCell_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help CircleCell

% Last Modified by GUIDE v2.5 28-Sep-2021 16:29:29

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @CircleCell_OpeningFcn, ...
    'gui_OutputFcn',  @CircleCell_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before CircleCell is made visible.
function CircleCell_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to CircleCell (see VARARGIN)

% Choose default command line output for CircleCell
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes CircleCell wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = CircleCell_OutputFcn(hObject, eventdata, handles)
...
if ~exist('AR','var') || isempty(AR)
    if ~exist('timeLapse','var') % load annotated project file
        ...
        if isfield(AR.FileInfo,'SettingFile')
            %load('C:\Users\Fan\Documents\MATLAB\CircleCell\DotTracker3.0\Settings1.mat')
        else
            %load(SettingFile)
        end
        % load the setting file
        %AR.Settings.Contrastx=Contrastx;AR.Settings.Offset=Offset;AR.Settings.DotSize=DotSize;
        AR.Settings.Threshold=0.98;%This used to be Threshold, but seems good to be 0.99
...
AR.FileInfo.SavingFile=[folder,'CICI-Analysis.mat'];
    ...
    for kc=1:ChnNum
        ...
        if 0 && ismember(kc,AR.Settings.AnalysisChn) % analyzed fluorescence channel
            if isfield(AR.DotInfo(kp),'DotChn') && ~isempty(AR.DotInfo(kp).DotChn) && isfield(AR.DotInfo(kp).DotChn(kf),'dot_id')
                ...
                %index=1:length(dotx);
                plot(handles.axes1,doty,dotx,'o','MarkerSize',AR.Settings.DotSize(kc)+4,'MarkerEdgeColor',color,'LineWidth',1)
                %for k=1:numel(dotx)
